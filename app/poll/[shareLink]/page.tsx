...
export default function PollParticipationPage() {
  ...
  const triggerAutoDraw = useCallback(
    async (pollId: string) => {
      try {
        console.log("Triggering auto-draw for poll:", pollId);
        ...
        console.log("Auto-draw successful:", data);

        // Force refresh all data after drawing
  ...
  const checkForResults = useCallback(async () => {
    ...
    try {
      ...
      const { data: pollData, error: pollError } = await supabase
        .from("polls")
        .select("results_drawn, is_active, open_until")
        .eq("id", poll.id)
        .single();
      ...
      if (hasExpired && !pollData.results_drawn) {
        console.log(
          "Poll has expired and no results drawn - triggering auto-draw!"
        );

        try {
          await triggerAutoDraw(poll.id);
